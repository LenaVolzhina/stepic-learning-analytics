...
def make_xes_without_actions(source_file_name, result_file_name, small = False):
    with open(result_file_name, 'w') as xes_file:
        ...
        with open(source_file_name, 'r') as csv_file:
            ...
            for line in csv_file:
                ...
                if small and int(new_step_no) > 40:
                    continue

                new_event = []

                # name of event:
                event_name = "{1}".format(old_step_no, step_type, action, new_step_no)
                new_event.append(["string", "concept:name", event_name])
                new_event.append(["date", "time:timestamp", timestamp])

                new_event.append(["string", "step_no", new_step_no])
                ...
                # добавляем его в trace, только если предыдущий event не про этот степ
                if len(trace_events) > 0 and trace_events[-1][2][2] != new_step_no:
...
result_file = "D:\stepic\data\\usual xes\\67_no_actions_step_type_small.xes"
make_xes_without_actions(source_file, result_file, small=True)
print("67 done")

source_file = "D:\stepic\data\\70_logs.csv"
result_file = "D:\stepic\data\\usual xes\\70_no_actions_step_type_small.xes"
make_xes_without_actions(source_file, result_file, small=True)
print("70 done")